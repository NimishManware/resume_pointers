...
def create_weights(data_dictionary, lambda_val):
    ...
    brack = XtX + lambda_val * np.identity(XtX.shape[0])

    inverse = np.linalg.inv(brack)
    weights = np.matmul(inverse,np.transpose(X))
...
def generate_test_error(data_dictionary, weights):
    ...
    Xw = np.matmul(X,weights)

    norm = np.linalg.norm(Xw - y) 
    test_error = norm ** 2 / y.size
...
def find_optimal_lambda(error_array, lambda_array):
    ...
    optimal_lambda = lambda_array[np.argmin(error_array)]
...
def plot_errors(lambda_array, error_array):
    '''
    ...
    plt.plot(error_array,lambda_array)
    plt.show()
    # END TODO
