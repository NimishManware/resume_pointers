...
def lasso_loss(X, Y, w, lambd):
    ...
    norm = torch.linalg.norm(Xw - Y) 
    norm = norm ** 2
    norm = norm / X.shape[0]
    loss = norm + lambd * torch.linalg.norm(w,1)
...
def lasso_loss_derivative(X, Y, w, lambd):
    ...
    derivative_1 = torch.matmul(Xt_X,w) - Xt_Y
    derivative_1 = 2 * derivative_1 /X.shape[0]

    derivative_2 = torch.sign(w)
    
    derivative = derivative_1 + lambd * derivative_2
