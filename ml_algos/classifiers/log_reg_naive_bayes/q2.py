...
class MultiClassLogisticRegression:
    ...
    def softmax(self, z):
        ...
        # exponents = np.exp(self.predict(self,z))
        post_softmax = np.exp(z)
        post_softmax = post_softmax / np.sum(post_softmax,axis=1,keepdims= True)
    ...
    def cross_entropy(self, y, probs):
        ...
        new_mat = np.multiply(y ,np.log(probs))
        post_cross_entropy = (-1) * new_mat.sum() / y.shape[0] 
        ### YOUR CODE ENDS HERE ###
