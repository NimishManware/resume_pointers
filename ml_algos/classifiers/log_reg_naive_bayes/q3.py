	...
	def predict(self, X):
		...
		for query in X:
			...
			for outcome in np.unique(self.y_train):
				...
				prior = self.class_priors #TODO_1   Take value from task-1 variable  (Remove 'None')
				likelihood = 1
				evidence = 1
				...
				posterior = prior[outcome] * likelihood / evidence #TODO_2         Remove 0 and get posterior

				probs_outcome[outcome] = posterior
				
		
			result = max(probs_outcome,key = lambda x:probs_outcome[x]) #TODO_3          Remove None and get the Max
