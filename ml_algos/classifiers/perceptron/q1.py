...
class Perceptron():
    ...
    def fit(self, x, y, plot_flag, lr = 0.001, epochs = 100):
        ...
        for epoch in range(epochs):
            ...
            y_pred = np.matmul(x,np.transpose(self.weights))
            # if epoch == 0:
            #     print("here",y_pred)
            # TODO perform the weight update

            self.weights = self.weights + lr * np.matmul(np.transpose(y[y*y_pred <= 0]), x[y*y_pred <= 0]) 
            

            # TODO update the running average of weights
            if epoch != 0:
                self.running_avg_weights = self.running_avg_weights*self.lam + self.weights*(1-self.lam)
            else:
                self.running_avg_weights = self.weights
    ...
    def predict(self, x, running_avg = False):
        ...
        '''
        #TODO concatenate 1's at the end of x to make it of the shape (data_size, input_dim+1) so that w[-1] can be the bias 

        x = np.column_stack((x,np.ones(x.shape[0])))

        # TODO make y_pred using either the final weight vector or the moving average of the weights
        if running_avg:
            y_pred = np.sign(np.matmul(x,np.transpose(self.running_avg_weights)))
        else:
            y_pred =  np.sign(np.matmul(x,np.transpose(self.weights)))
        return y_pred
