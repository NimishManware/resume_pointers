...
def gini_index(classes):
    ...
    probability = counts / classes.size

    # Calculate the gini index value using the formula: 1 - Σ(p_i^2)
    gini_impurity_value = 1 - (probability ** 2).sum()
...
def entropy(classes):
    ...
    entropy_value = -(np.dot(probability,np.log2(probability)))
...
def information_gain(examples, classes, attr):
    ...
    uniques,counts = np.unique(examples[:,attr],return_counts=True)

    # Calculate the entropy of the entire dataset before splitting

    entropy_before = entropy(classes)
    ...
    for i in range(uniques.size):
        entropy_after += probability[i] * entropy(classes[examples[:,attr] == uniques[i]])
...
def gini_gain(example, classes, attr):
    ...
    uniques,counts = np.unique(example[:,attr],return_counts=True)

    # Calculate the gini index of the entire dataset before splitting

    gini_before = gini_index(classes)
    ...
    gini_after = 0
    for i in range(uniques.size):
        gini_after += probability[i] * gini_index(classes[example[:,attr] == uniques[i]])
    # Calculate and return the gini gain

    gini_gain_value = gini_before - gini_after
...
class DecisionTree:
    ...
    def __build_tree__(self, features, classes, attributes, depth=0):
        ...
        if (len(attributes) == 0):
            ...
            max_ig = -1000
            for i in attributes:
                uniques = np.unique(features[:,i])

                ig = information_gain(features,classes,i)
            ...
            uniques = np.unique(features[:,best_attr])

            # Create a dictionary to hold child nodes.
            children = {}

            # Remove the best attribute from the list of attributes to pass to child nodes.
            rem_attr = [i for i in attributes if i != best_attr]
            

            # Split the dataset based on the unique values of the best attribute.
            for v in uniques:
                children[v] = self.__build_tree__(features[features[:,best_attr] == v],classes[features[:,best_attr] == v],rem_attr,depth+1)
