...
def simplex_solver(A_matrix: numpy.array, c: numpy.array, b: numpy.array) -> list:
    ...
    ################################################################
    # %% Student Code Start
    slack_var = numpy.identity(c.size)
    table = numpy.hstack((A_matrix,slack_var))
    b = numpy.expand_dims(b, axis=-1)
    basic_variables = [(-1)*i for i in range(c.size)]
    op_func = [-x for x in c] + [ 0 for i in range(c.size + 1)] 
    table = numpy.hstack((table,b))
    table = numpy.vstack((table,op_func))
    while (table[-1,:].min() < 0):
        min_col = numpy.argmin(table[-1,:])
        min_row = -1
        min_theta = 999999
        for i in range(c.size):
            if table[i,min_col] != 0 :
                if min_theta > table[i,-1]/table[i,min_col] > 0:
                    min_row = i
                    min_theta = table[i,-1]
        pivot_value_list.append(table[min_row,min_col].round(3))
        table[min_row,:] = table[min_row,:]/table[min_row,min_col]
        for i in range(c.size + 1):
            if i != min_row:
                table[i,:] = table[i,:] - table[min_row,:] * table[i,min_col]
    # %% Student Code End
    ################################################################
