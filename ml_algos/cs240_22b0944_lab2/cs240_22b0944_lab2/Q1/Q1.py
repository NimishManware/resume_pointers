...
def w_closed_form(X, Y):
    ...
    Xt_X = torch.matmul(np.transpose(X),X)
    if torch.linalg.det(Xt_X) != 0:
        inv = torch.linalg.inv(Xt_X)
    else:
        inv = torch.linalg.inv(Xt_X)
    w_closed = torch.matmul(inv,np.transpose(X))
    w_closed = torch.matmul(w_closed,Y)
    np.round(w_closed,4)
...
def l2_loss_derivative(X, Y, w):
    ...
    Xt_Y = torch.matmul(np.transpose(X),Y)
    derivative = torch.matmul(Xt_X,w) - Xt_Y
...
def train_model(X_train, Y_train, X_test, Y_test, w, eta):
    ...
    @returns
        ...
        iters : Total iterations it take for algorithm to converge (NO NEED)
